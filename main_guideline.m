...
%idx1 = randsample(N,ceil(N*frac),1); % A new training set for each tree is generated by random sampling from dataset WITH replacement.
idx1 = randsample(N,ceil(N),1);
plot_toydata(data_train(idx1,:));
unique_percentage1 = 100.*length(unique(idx1))./N;        %Calculating unique percentage of each dataset
str = sprintf('Data Set 1, Uniqueness, %.1f%%', unique_percentage1);
set(gca, 'fontsize', 10);
title(str, 'FontSize', 10 );

subplot(2,2,2)       % add first plot in 2 x 1 grid
idx2 = randsample(N,ceil(N),1); % A new training set for each tree is generated by random sampling from dataset WITH replacement.
plot_toydata(data_train(idx2,:));
unique_percentage2 = 100.*length(unique(idx2))./N;        %Calculating unique percentage of each dataset
str = sprintf('Data Set 2, Uniqueness, %.1f%%', unique_percentage2);
set(gca, 'fontsize', 10);
title(str, 'FontSize', 10 );

subplot(2,2,3)       % add first plot in 2 x 1 grid
idx3 = randsample(N,ceil(N),1); % A new training set for each tree is generated by random sampling from dataset WITH replacement.
plot_toydata(data_train(idx3,:));
unique_percentage3 = 100.*length(unique(idx3))./N;        %Calculating unique percentage of each dataset
str = sprintf('Data Set 3, Uniqueness, %.1f%%', unique_percentage3);
set(gca, 'fontsize', 10);
title(str, 'FontSize', 10 );

subplot(2,2,4)       % add first plot in 2 x 1 grid
idx4 = randsample(N,ceil(N),1); % A new training set for each tree is generated by random sampling from dataset WITH replacement.
plot_toydata(data_train(idx4,:));
unique_percentage4 = 100.*length(unique(idx4))./N;        %Calculating unique percentage of each dataset
str = sprintf('Data Set 4, Uniqueness, %.1f%%', unique_percentage4);
set(gca, 'fontsize', 10);
title(str, 'FontSize', 10 );

fig = gcf;
fig.PaperPositionMode = 'auto';
print('4_subsets_bagging','-depsc','-r0');
